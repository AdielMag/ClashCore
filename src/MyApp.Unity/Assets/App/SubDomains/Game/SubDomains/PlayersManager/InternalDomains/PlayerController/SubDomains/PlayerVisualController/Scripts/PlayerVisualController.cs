...
namespace App.SubDomains.Game.SubDomains.MovementAnimator.Scripts
{
    public class PlayerVisualController : MonoBehaviour
    {
        ...
        private static readonly int _sLookAtHash = Animator.StringToHash("LookAt");
        ...
        {
            var hasTarget = lookAtTarget;
            if (hasTarget)
            {
                _lateUpdateAction = UpdateLookAtParameters;
            }
            else
            {
        ...
        private void UpdateLookAtParameters()
        {
            // Cache current transform state
            _currentPosition = transform.position;
            _currentRotation = transform.rotation;
            ...
            // Calculate target values (normalize by time to handle varying frame rates)
            var rawVertical = localDelta.z / Time.deltaTime;
            var rawHorizontal = localDelta.x / Time.deltaTime;

            // Normalize and clamp the values
            var targetVertical = Mathf.Clamp(rawVertical / _playerPhysicsSettings.Speed, -1f, 1f);
            var targetHorizontal = Mathf.Clamp(rawHorizontal / _playerPhysicsSettings.Speed, -1f, 1f);

            // Smooth the values
            float verticalVelocity = 0f;
            float horizontalVelocity = 0f;
            float currentVertical = Mathf.SmoothDamp(animator.GetFloat(_sVerticalHash), targetVertical, ref verticalVelocity, smoothTime);
            float currentHorizontal = Mathf.SmoothDamp(animator.GetFloat(_sHorizontalHash), targetHorizontal, ref horizontalVelocity, smoothTime);

            // Update animator
            animator.SetFloat(_sVerticalHash, currentVertical);
            animator.SetFloat(_sHorizontalHash, currentHorizontal);
